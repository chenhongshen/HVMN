...
class Sampler(object):
    ...
    def sample(self, *args, **kwargs):
        ...
        if self.model.add_memory:
            prev_mem_block = numpy.zeros((n_samples, self.model.memory_size, self.model.memory_dim), dtype='float32')
        if not self.model.reset_utterance_decoder_at_end_of_utterance:
        ...
        for k in range(max_length):
            ...
            # Recompute encoder states, hs, random variables and memory blocks 
            ...
            if len(indx_update_hs):
                #HongshenToDo 
                encoder_states = self.compute_encoding(context[:, indx_update_hs], reversed_context[:, indx_update_hs], 2500)
                prev_hs[indx_update_hs] = encoder_states[1][-1]
                if self.model.add_memory:
                    prev_mem_block[indx_update_hs] = encoder_states[2][-1]
            ...
            # ... done
            if self.model.add_memory:
                next_probs, new_hd = self.next_probs_predictor(prev_hs, prev_hd, prev_words, context, ran_vectors, prev_mem_block)
            else:
            ...
            if self.model.add_memory:
                prev_mem_block = prev_mem_block[new_sources]
