...
def main(args):     
    ...
    if args.pretrained != "":
        filename = args.pretrained + '_model.npz'
        if os.path.isfile(filename):
            logger.debug("Loading previous model")
            ...
            if args.reinitialize_memory:
                parameter_strings_to_ignore += ['memory']

                save_model_on_first_valid = True

            load(model, filename, parameter_strings_to_ignore)
        else:
            raise Exception("Cannot start from pretrained, cannot find model file!")
        # assign new run_id key
        model.state['run_id'] = RUN_ID
    ...
    while (step < state['loop_iters'] and
        ...
        if step % 200 == 0:
            ...
            print "Sampled : {}".format(samples[0][0])
...
def parse_args():
    ...
    parser.add_argument("--pretrained",type=str,default="",help="Read data from pretrained model")
    ...
    parser.add_argument("--reinitialize-latent-variable-parameters", action='store_true', help="Can be used when resuming or start training from a pretrained model. If true, will initialize all latent variable parameters randomly instead of loading them from previous model.")

    parser.add_argument("--reinitialize-decoder-parameters", action='store_true', help="Can be used when resuming or start training from a pretrained model. If true, will initialize all parameters of the utterance decoder randomly instead of loading them from previous model.")
    
    parser.add_argument("--reinitialize_memory", action='store_true', help="Can be used when resuming or start training from a pretrained model. If true, will initialize all parameters of the memory randomly instead of loading them from previous model.")
