    ...
    def init_params(self): 
        ...
        if self.add_latent_gaussian_per_utterance:
            if self.condition_decoder_only_on_latent_variable:
                if self.add_memory:
                    self.input_dim = self.memory_dim
                else:
...
class Memory(object):
    ...
    """
    def init_params (self):
        self.W_c_f_mc = add_to_params(self.params,theano.shared(value=NormalInit(self.rng, self.memory_dim, self.memory_dim), name= 'W_c_f_mc'+self.name))
        self.W_c_f_h = add_to_params(self.params,theano.shared(value=NormalInit(self.rng, self.input_dim, self.memory_dim), name= 'W_c_f_h'+self.name))
        self.W_c_i_mc = add_to_params(self.params,theano.shared(value=NormalInit(self.rng, self.memory_dim, self.memory_dim), name= 'W_c_i_mc'+self.name))
        self.W_c_i_h = add_to_params(self.params,theano.shared(value=NormalInit(self.rng, self.input_dim, self.memory_dim), name= 'W_c_i_h'+self.name))
        self.W_c_u_mc = add_to_params(self.params,theano.shared(value=NormalInit(self.rng, self.memory_dim, self.memory_dim), name= 'W_c_u_mc'+self.name))
        self.W_c_u_h = add_to_params(self.params,theano.shared(value=NormalInit(self.rng, self.input_dim, self.memory_dim), name= 'W_c_u_h'+self.name))
        
        self.W_mem_f = add_to_params(self.params,theano.shared(value=NormalInit(self.rng, self.memory_dim, self.memory_dim), name= 'W_mem_f'+self.name))
        self.W_mem_i = add_to_params(self.params,theano.shared(value=NormalInit(self.rng, self.memory_dim, self.memory_dim), name= 'W_mem_i'+self.name))
        self.V_mem_f = add_to_params(self.params, theano.shared(value=np.asarray(self.rng.uniform(low=.5,high=.5,size=(self.memory_dim,)), dtype = 'float32'), name='V_mem_f'+self.name))
        self.b_mem_f = add_to_params(self.params, theano.shared(value=np.zeros((self.memory_size,), dtype='float32'), name='b_mem_f'+self.name))
        self.V_mem_i = add_to_params(self.params, theano.shared(value=np.asarray(self.rng.uniform(low=.5,high=.5,size=(self.memory_dim,)), dtype = 'float32'), name='V_mem_i'+self.name))
        self.b_mem_i = add_to_params(self.params, theano.shared(value=np.zeros((self.memory_size,), dtype='float32'), name='b_mem_i'+self.name))
        self.b_mem_u = add_to_params(self.params, theano.shared(value=np.zeros((self.memory_dim,), dtype='float32'), name='b_mem_u'+self.name))
    def read_write_step (self, i_t, h_t, m_t, mem_tm1):
        if m_t.ndim >= 1:
            m_t = m_t.dimshuffle(0, 'x')
        #read
        #i_t:batch,mem_size
        i_t = i_t.dimshuffle(0, 1, 'x')
        #mc:batch,mem_size,mem_dim
        mc = mem_tm1 * i_t
        #mc:batch,mem_dim
        mc = mc.sum(axis = 1)
        #mc = m_t * mc + (1 - m_t) * mc
        ...
        #h_t:batch,input_dim
        #c_f:batch,mem_dim
        c_f = T.dot(mc,self.W_c_f_mc) + T.dot(h_t, self.W_c_f_h)
        #c_i:batch,mem_dim
        c_i = T.dot(mc,self.W_c_i_mc) + T.dot(h_t, self.W_c_i_h)
        #c_u:batch,mem_dim
        c_u = T.dot(mc,self.W_c_u_mc) + T.dot(h_t, self.W_c_u_h)
        #c_f:batch,mem_dim
        #f_gate: batch,mem_size
        #        batch,mem_size,mem_dim dot mem_dim  +mem_size 
        c_f = c_f.dimshuffle(0,'x',1)
        f_gate = T.nnet.sigmoid(T.dot(c_f + T.dot(mem_tm1, self.W_mem_f ), self.V_mem_f) + self.b_mem_f)
        #i_gate: batch,mem_size
        c_i = c_i.dimshuffle(0,'x',1)
        i_gate = T.nnet.sigmoid(T.dot(c_i + T.dot(mem_tm1, self.W_mem_i ), self.V_mem_i) + self.b_mem_i)
        #update: batch,mem_dim
        update = T.tanh(c_u + self.b_mem_u)
        #mem_t: batch,mem_size,dim
        mem_t = mem_tm1*f_gate.dimshuffle(0, 1, 'x')+\
                i_gate.dimshuffle(0, 1, 'x')*update.dimshuffle(0,'x',1)
        m_tt = m_t.dimshuffle(0, 1, 'x')
        mem_t = (1 - m_tt) * mem_t + (m_tt) * mem_tm1
        return mem_t,mc
        
    def read_write (self, h, index, x, xmask = None, prev_mem = None, **kwargs):
        """
        Read Memory by Index and write it with x
        """
        one_step = False
        if len(kwargs):
            one_step = True
        ...
        if not one_step:
            if prev_mem:
                ...
                mem_0 = T.alloc(np.float32(0), batch_size, self.memory_size, self.memory_dim)

        # sampling mode (i.e. one step)
        else:
            # in this case x.ndim != 2
            assert x.ndim != 2
            assert 'prev_mem' in kwargs
            mem_0 = kwargs['prev_mem']

        if xmask == None:
            xmask = T.neq(x, self.eos_sym)

        f_hier = self.read_write_step
        o_hier_info = [mem_0,None]
        ...
        if not one_step:
            [mem_updates,mem_out], _ = theano.scan(f_hier,\
                               sequences=[index, h, xmask],\
                               outputs_info=o_hier_info)

        # Just one step further
        else:
            mem_updates,mem_out = f_hier(h_out, xmask, mem_0)

        return mem_updates, mem_out 
            
    
    def __init__(self, state, memory_size, memory_dim, input_dim, rng, parent, name):
        self.rng = rng
        self.__dict__.update(state)
...
class DialogEncoderDecoder(Model):
    ...
    def build_next_probs_function(self):
        if not hasattr(self, 'next_probs_fn'):
            ...
            if self.add_latent_gaussian_per_utterance:
                #beam_hs: n_samples,batch
                if self.condition_latent_variable_on_dialogue_encoder:
                    #HS: ('x',0,1) is right op, but (0,'x',1) also runs OK				
                ...
                if self.add_memory:
                    _, mem_info = self.memory.read_write(\
                        self.hs_to_condition_latent_variable_on, \
                        prior_sample.dimshuffle((0, 'x', 1)), \
                        self.beam_x_data, \
                        prev_mem=self.beam_memblock)
                    mem_info=mem_info[0]

                if self.condition_decoder_only_on_latent_variable:
                    if self.add_memory:
                        decoder_inp = mem_info
                    else:
                        decoder_inp = prior_sample
                else:
                    if self.add_memory:
                        decoder_inp = T.concatenate([self.beam_hs, mem_info], axis=1)
                    else:
            ...
            if self.add_memory:
                self.next_probs_fn = theano.function(inputs=[self.beam_hs, self.beam_hd, self.beam_source, self.beam_x_data, self.beam_ran_cost_utterance, self.beam_memblock],
                outputs=[outputs, hd],
                on_unused_input='warn',
                name="next_probs_fn")
            else:
    ...
    def build_encoder_function(self):
        if not hasattr(self, 'encoder_fn'):
            ...
            if self.add_latent_gaussian_per_utterance:
                ...
                if self.add_memory:
                    mem_block_updates, mem_info = self.memory.read_write(
                    hs_to_condition_latent_variable_on,\
                    latent_utterance_variable_approx_posterior_mean,\
                    self.x_data)
            ...
            if self.direct_connection_between_encoders_and_decoder:
                ...
                if self.add_latent_gaussian_per_utterance:
                    if self.condition_decoder_only_on_latent_variable:
                        ...
                        if self.add_memory:
                            hd_input = T.concatenate([hs, hs_dummy, mem_info], axis=2)
                        else:
                ...
                if self.add_latent_gaussian_per_utterance:
                    if self.condition_decoder_only_on_latent_variable:
                        ...
                        if self.add_memory:
                            hd_input = T.concatenate([hs, mem_info], axis=2)
                        else:
            ...
            if self.add_latent_gaussian_per_utterance:
                if self.add_memory:
                    logger.debug("Build Encoder Function")
                    self.encoder_fn = theano.function(inputs=[self.x_data, \
                            self.x_data_reversed, self.x_max_length], \
                            outputs=[h, hs_complete, mem_block_updates, hd], on_unused_input='warn', name="encoder_fn")
                else:
                    self.encoder_fn = theano.function(inputs=[self.x_data, \
                        self.x_data_reversed, self.x_max_length], \
    ...
    def __init__(self, state):
        ...
        if not 'add_memory' in state:
           state['add_memory'] = False
...
# * self.x_cost_mask[1:self.x_max_length]) * (T.sum(T.eq(training_x, self.eos_sym)) / (T.sum(training_x_cost_mask_flat)))
        ...
        if self.add_memory:
            self.prev_mem_state = theano.shared(value=numpy.zeros((self.bs, self.memory_size, self.memory_dim), dtype='float32'), name='prev_mem_state')
            self.memory = Memory(self.state, self.memory_size, self.memory_dim, self.sdim, self.rng, self, 'memory')
            _,self.mem_info = self.memory.read_write(\
                self.hs,\
                self.posterior_sample,\
                training_x, \
                xmask = training_hs_mask, \
                prev_mem = self.prev_mem_state)#
        ...
        if self.direct_connection_between_encoders_and_decoder:
            ...
            if self.add_latent_gaussian_per_utterance:
                if self.condition_decoder_only_on_latent_variable:
                    if self.add_memory:
                        self.hd_input = self.mem_info
                    else:
                        self.hd_input = self.posterior_sample
                else:
                    if self.add_memory:
                        self.hd_input = T.concatenate([self.hs, self.hs_dummy, self.mem_info], axis=2)
                    else:
            ...
            if self.add_latent_gaussian_per_utterance:
                if self.condition_decoder_only_on_latent_variable:
                    ...
                    if self.add_memory:
                        self.hd_input = T.concatenate([self.hs, self.mem_info], axis=2)
                    else:
        ...
        if self.add_latent_gaussian_per_utterance:
            ...
            if self.add_memory:
                assert len(set(self.params)) + len(set(self.memory.params)) \
                    == len(set(self.params +self.memory.params))
        ...
        self.beam_memblock = T.tensor3("beam_memblock")