            ...
            if load_parameter:
                if p.name in vals:
                    ...
                    if p.get_value().shape != vals[p.name].shape:
                        if len(p.get_value().shape)==1 or (p.get_value().shape[1] == vals[p.name].shape[1]):
                            tmp=p.get_value()
                            pre_wnum=vals[p.name].shape[0]
                            tmp[:pre_wnum]=vals[p.name]
                            p.set_value(tmp)
                            logger.error('Partial Init {}'.format(p.name))
                            continue
                        else:
